...
STATUS_FILE_PATH = "/home/huazheng/DSLB_EESM/dynamic_load_balancing/active_servers_status.json"

def load_server_status():
    """Loads the status file created by the scaling controller on Server 2."""
    try:
        with open(STATUS_FILE_PATH, 'r') as f:
            return json.load(f)
    except (FileNotFoundError, json.JSONDecodeError):
        # This will happen if the file hasn't been synced yet, or Ansible failed.
        # It's a non-fatal warning, allowing DWRS to continue but without draining knowledge.
        print(f"Warning: Server status file not found or corrupted at {STATUS_FILE_PATH}. Proceeding without draining awareness.")
        return {}
...
def update_server_weights():
    ...
    for ip, metrics in all_metrics.items():
        status = server_status.get(ip, {})
        
        # ***CRITICAL FILTERING STEP: EXCLUDE DRAINING SERVERS***
        is_draining = status.get("draining", False)
        
        if is_draining:
            print(f"Skipping server {ip}: Currently in draining state (excluded from load pool).")
            continue
...
def select_target_server(servers):
    ...
    if len(servers) == 1:
        ...
        print(f"Only one eligible server is active ({the_only_server['ip']}). Selecting it by default.")
        print(f"*** Target Server Selected: {the_only_server['ip']} ***")
        return the_only_server
    
    servers.sort(key=lambda x: x['dynamic_weight'], reverse=True)

    # 1. Calculate the sum of all weights
    total_weight = sum(server['dynamic_weight'] for server in servers)
    
    if total_weight == 0:
        print("Cannot select a server: Total weight of all servers is 0. Falling back to random selection.")
